import template from './manager.findTutor.html';
	...
	controller: function ($scope, authenticationService) {
		...
		$scope.goToTutorPage = function (r) {
			...
			$scope.register = function () {
				if($scope.reg.student && $scope.reg.subject && $scope.reg.time){

					$scope.reg.date.setHours($scope.reg.time.hour, $scope.reg.time.min);
					var data = {
						studentId: $scope.reg.student,
						username: $scope.tutorUsername,
						subjectId: $scope.reg.subject,
						date: $scope.reg.date

					}
					authenticationService.registerSession(data).then(function (response) {
						alert("successfully registered student for a session");
						$scope.reg = {};
					},function (err) {
						alert("Failed to register your student");
					})
				}
			}
		}
